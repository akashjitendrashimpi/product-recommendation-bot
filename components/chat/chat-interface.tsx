...
import { QrCode, ShoppingBag, ArrowRight, RotateCcw, ExternalLink, Store } from "lucide-react"
...
export function ChatInterface({ campaign, products, categories, user }: ChatInterfaceProps) {
  ...
  // Increment scan count only once when component mounts (prevents double increment in React Strict Mode)
  useEffect(() => {
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Welcome{user?.display_name ? ` to ${user.display_name}` : ""}!
              </CardTitle>
              <CardDescription>
                {products.length > 0
                  ? "Answer a few questions and I'll recommend the perfect products for you."
                  : "This user hasn't added any products yet. Please check back later!"}
              </CardDescription>
            </CardHeader>
            {products.length > 0 && (
