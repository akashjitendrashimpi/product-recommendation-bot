...
import {
  ...
  Edit, Camera, Trophy, Star, TrendingUp, Copy, AlertCircle,
  IndianRupee, Calendar, Clock
} from "lucide-react"
...
export function ProfilePage({ user: initialUser }: ProfilePageProps) {
  ...
  const getLevel = () => {
    ...
    if (e >= 5000) return { label: "Diamond", color: "text-blue-500", bg: "bg-blue-100", next: null }
    if (e >= 2000) return { label: "Gold", color: "text-yellow-500", bg: "bg-yellow-100", next: 5000 }
    if (e >= 500) return { label: "Silver", color: "text-gray-500", bg: "bg-gray-100", next: 2000 }
    return { label: "Bronze", color: "text-orange-500", bg: "bg-orange-100", next: 500 }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card className="border-0 shadow-lg rounded-3xl overflow-hidden">
        <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-purple-700 p-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${level.bg} ${level.color}`}>
                  {level.label === "Diamond" ? "💎" : level.label === "Gold" ? "🥇" : level.label === "Silver" ? "🥈" : "🥉"} {level.label}
                </span>
                {user.is_admin && (
  ...
  <div className="mt-5">
    <div className="flex justify-between mb-1.5">
      <span className="text-blue-200 text-xs">Progress to next level</span>
    ...
    <div className="w-full bg-white/20 rounded-full h-2">
      ...
      />
        ...
        <div className="grid grid-cols-4 divide-x divide-gray-100">
          {[
            { label: "Earned", value: `₹${stats.totalEarnings.toFixed(0)}`, icon: IndianRupee },
            { label: "Tasks", value: stats.tasksCompleted, icon: Trophy },
            { label: "Referrals", value: stats.totalReferrals, icon: Star },
            { label: "Days", value: stats.memberDays, icon: Calendar },
          ].map((s, i) => (
      ...
      <Card className="border border-gray-200 rounded-2xl shadow-sm">
        <CardContent className="p-0 divide-y divide-gray-100">
          ...
          <ProfileField
            ...
            isLoading={isLoading}
            badge={user.upi_id ? <Badge className="bg-green-100 text-green-700 text-xs">✓ Verified</Badge> : <Badge className="bg-yellow-100 text-yellow-700 text-xs">⚠ Required</Badge>}
          >
            <Input
          ...
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-gray-500">Share and earn ₹20 per friend</p>
              </div>
              <Badge className="bg-green-600 text-white">₹20/referral</Badge>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-white border border-green-200 rounded-xl px-4 py-3 font-mono font-bold text-gray-900 text-lg tracking-widest">
                {user.referral_code}
              </div>
      ...
      <Card className="border border-gray-200 rounded-2xl shadow-sm">
        <CardContent className="p-5 space-y-3">
          <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide text-gray-500">Account Info</h3>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">Member since</span>
            <span className="text-sm font-semibold text-gray-900">
              {user.created_at ? new Date(user.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" }) : "N/A"}
            </span>
          </div>
          <div className="flex items-center justify-between">
    ...
    <div className="p-4">
      {editing ? (
        ...
        <div className="flex items-center gap-3">
          ...
          {!readonly && (
            <button onClick={onEdit} title={`Edit ${label}`} aria-label={`Edit ${label}`}
              className="text-blue-600 hover:text-blue-700 p-1.5 rounded-lg hover:bg-blue-50">
